...
class LogisticLayer():
    ...
    def forward(self, inp):
        ...
        for outputIndex in range(0, self.n_out):
            #Skalarprodukt
            foo = numpy.dot(inp, self.weights[outputIndex])
            outp[outputIndex] = self._fire(foo)
        return outp

    def computeDerivative(self, nextDerivatives, nextWeights, inputTargets):
        """
        ...
        first = numpy.sub(inputTargets, self.outp)
        second = self.outp
        third = numpy.sub(1, inputTargets)
        returnvalue = numpy.mult(first, second, third, self.inp)
        return returnvalue

    def updateWeights(self, learningRate, nextDerivatives, nextWeights, inputTargets):
        """
        ...
        if self.is_classifier_layer:
            ...
            deltas = numpy.multiply(learningRate,
                                    computeDerivative(nextDerivatives,
                                                      nextWeights, inputTargets))
            self.weight = numpy.sum(self.weight, deltas)
        else:
            pass
